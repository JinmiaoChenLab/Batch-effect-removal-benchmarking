                         ...
                         visualize = T, save_obj = T)
{
  ...
  cosd4 <- cosineNorm(as.matrix(b4_data))
  cosd5 <- cosineNorm(as.matrix(b5_data))

  pcs_total <- multiBatchPCA(cosd1, cosd2, cosd3, cosd4, cosd5)   #defaut npca = 50
  out_mnn_total <- fastMNN(pcs_total[[1]], pcs_total[[2]], pcs_total[[3]], pcs_total[[4]], pcs_total[[5]], pc.input = TRUE)
