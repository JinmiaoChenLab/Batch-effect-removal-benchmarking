...
read_dir = "/home/hoa/hoatran/demo_normalization/dataset/dataset10_hematoMNN_Hoa/final/"
...
#batchlabel_v = c(b1_batch_label,b2_batch_label)
#celltype_v = c(b1_celltype,b2_celltype)
# data_meta = rbind(b1_metadata, b2_metadata)
...
setwd(this.dir)
data_dir <- '/home/hoa/hoatran/demo_normalization/dataset/dataset10_hematoMNN_Hoa/final/'
base_name <- 'combat_dataset10/'
...
myData <- cbind(b1_exprs,b2_exprs)
mySample <- rbind(b1_metadata, b2_metadata)
mySample$celltype <- mySample$CellType

opt1 <- './combat_dataset10/parametric/'
# opt2 <- './combat_dataset10/non-parametric/'
...
myFilteredData <- filter_data_mtx(myData, base_name, is_filter_cells=FALSE,min_genes=300, 
...
#                             base_name=opt2, save_txt=TRUE, saveRDA=TRUE)
...
combat_srt <- CreateSeuratObject(raw.data = combat_output, project = "dataset10_combat")
dim(combat_srt@raw.data)  # 3467 4649
