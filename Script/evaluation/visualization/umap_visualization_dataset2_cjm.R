...
this.dir <- "X:/acrc/jinmiao/CJM_lab/hoatran/demo_normalization/manuscript_results/visualization/"
setwd(this.dir)

input_path = "X:/acrc/jinmiao/CJM_lab/hoatran/demo_normalization/xiaomeng/generate_PCA_tSNE_UMAP/dataset2/"
raw_path = "X:/acrc/jinmiao/CJM_lab/hoatran/demo_normalization/manuscript_results/dataset2_cellatlas/visualization/raw/"
bbknn_path = "X:/acrc/jinmiao/CJM_lab/hoatran/demo_normalization/manuscript_results/dataset2_cellatlas/visualization/bbknn/"
...
seurat2_df <- read.csv(paste0(input_path,dataset,"_seurat2_umap.csv"), head=T, row.names = 1, check.names = FALSE)
seurat2_df=correct_name(seurat2_df)

harmony_df <- read.csv(paste0(input_path,dataset,"_harmony_umap.csv"), head=T, row.names = 1, check.names = FALSE)
harmony_df=correct_name(harmony_df)
  
scanorama_df <- read.csv(paste0(input_path,dataset,"_scanorama_umap.csv"),head=T, row.names = 1, check.names = FALSE)
scanorama_df=correct_name(scanorama_df)
# bbknn_df <- read.csv("bbknn_umap.csv", row.names = 1, check.names = FALSE)

scgen_df <- read.csv(paste0(input_path,dataset,"_scgen_umap.csv"),head=T, row.names = 1, check.names = FALSE)
scgen_df=correct_name(scgen_df)

resnet_df <- read.csv(paste0(input_path,dataset,"_resnet_umap.csv"),head=T, row.names = 1, check.names = FALSE)
resnet_df=correct_name(resnet_df)

raw_df <- read.csv(paste0(raw_path,"rawdata_umap.csv"),head=T, row.names = 1, check.names = FALSE)
#raw_df <- read.table(paste0(raw_path,"filtered_umap.txt"),head=T, row.names = 1, check.names = FALSE)

raw_df=correct_name(raw_df)
colnames(raw_df)[grep("UMAP1",colnames(raw_df))] = "umap1"
colnames(raw_df)[grep("UMAP2",colnames(raw_df))] = "umap2"

fastMNN_df <- read.csv(paste0(input_path,dataset,"_fastMNN_umap.csv"),head=T, row.names = 1, check.names = FALSE)
fastMNN_df=correct_name(fastMNN_df)

seurat3_df <- read.csv(paste0(input_path,dataset,"_seurat3_umap.csv"),head=T, row.names = 1, check.names = FALSE)
seurat3_df=correct_name(seurat3_df)

correctMNN_df <- read.csv(paste0(input_path,dataset,"_classicMNN_umap.csv"),head=T, row.names = 1, check.names = FALSE)
correctMNN_df=correct_name(correctMNN_df)

combat_df <- read.csv(paste0(input_path,dataset,"_combat_umap.csv"),head=T, row.names = 1, check.names = FALSE)
combat_df=correct_name(combat_df)

limma_df <- read.csv(paste0(input_path,dataset,"_limma_umap.csv"),head=T, row.names = 1, check.names = FALSE)
limma_df=correct_name(limma_df)

liger_df <- read.csv(paste0(input_path,dataset,"_liger_umap.csv"),head=T, row.names = 1, check.names = FALSE)
liger_df=correct_name(liger_df)

scmerge_df <- read.csv(paste0(input_path,dataset,"_scmerge_umap.csv"),head=T, row.names = 1, check.names = FALSE)
scmerge_df=correct_name(scmerge_df)

zinbwave_df <- read.csv(paste0(input_path,dataset,"_zinbwave_umap.csv"),head=T, row.names = 1, check.names = FALSE)
zinbwave_df=correct_name(zinbwave_df)

bbknn_df <- read.csv(paste0(bbknn_path,"bbknn_umap.csv"),head=T, row.names = 1, check.names = FALSE)
bbknn_df=correct_name(bbknn_df)

# seurat2_df <- read.table("./seurat2_multicca/seuratCCA_umap.txt", head=T, row.names = 1, check.names = FALSE)
# harmony_df <- read.table("./harmony/harmony_umap.txt", head=T, row.names = 1, check.names = FALSE)
# scanorama_df <- read.csv("./scanorama/scanorama_umap.csv", row.names = 1, check.names = FALSE)
# # bbknn_df <- read.csv("./bbknn/bbknn_umap.csv", row.names = 1, check.names = FALSE)
# scgen_df <- read.csv("./scGen/scGen_umap.csv", row.names = 1, check.names = FALSE)
# resnet_df <- read.csv("./resnet/resnet_umap.csv", row.names = 1, check.names = FALSE)
# raw_df <- read.table("./raw/filtered_umap.txt", head=T, row.names = 1, check.names = FALSE)
# fastMNN_df <- read.table("./fastMNN/fastMNN_umap.txt", head=T, row.names = 1, check.names = FALSE)
# seurat3_df <- read.table("./seurat3_integration/seurat3_umap.txt", head=T, row.names = 1, check.names = FALSE)
# correctMNN_df <- read.csv("./MNNCorrect/mnncorrect_scanpy_umap_cellatlas.csv", row.names = 1, check.names = FALSE)
# combat_df <- read.table("./combat/combat_corrected_dataset2_umap.txt", head=T, row.names = 1, check.names = FALSE)
# # limma_df <- read.table("./limma/limma_umap.txt", head=T, row.names = 1, check.names = FALSE)
# liger_df <- read.table("./liger/liger_umap.txt", head=T, row.names = 1, check.names = FALSE)
# scmerge_df <- read.csv("./scMerge/scmerge_umap.csv", row.names = 1, check.names = FALSE)
# # zinbwave_df <- read.table("./zinbwave/zinbwave_umap.txt", head=T, row.names = 1, check.names = FALSE)
...
plot_df$celltype[plot_df$celltype=="B-cell"] = "B cell"
plot_df$celltype[plot_df$celltype=="Smooth-muscle"] = "Smooth muscle"
plot_df$celltype[plot_df$celltype=="T-cell"] = "T cell"
###
...
plots <- list(raw_batch, seurat2_batch, seurat3_batch, harmony_batch, fastmnn_batch, correctmnn_batch, combat_batch, limma_batch,
              ...
              scgen_batch, scanorama_batch, resnet_batch, zinbwave_batch, scmerge_batch, liger_batch, bbknn_batch, legend1,
              scgen_cellt, scanorama_cellt, resnet_cellt, zinbwave_cellt, scmerge_cellt, liger_cellt, bbknn_cellt, legend2)
...
# bbknn have umap plot only, no umap
...
png(paste(base_name,"_umap.png",sep=""),height = 2*900, width=2*1700,res = 2*72)
# pdf(paste(base_name,"_umap.pdf",sep=""), height=plotheight, width=plotwidth)
print(grid.arrange(grobs = plots[select_grobs(layout)], layout_matrix = layout,
                   bottom=" ",right=" "))
#top="umap Visualization"
dev.off()

#png(paste(base_name,"_umap.png",sep=""),height = 2*900, width=2*1700,res = 2*72)
pdf(paste(base_name,"_umap.pdf",sep=""), height=2*900/300*2, width=2*1700/300*2)
print(grid.arrange(grobs = plots[select_grobs(layout)], layout_matrix = layout,
                   bottom=" ",right=" "))
#top="umap Visualization"
dev.off()
