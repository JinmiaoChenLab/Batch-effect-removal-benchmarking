...
run_ARISampled_6 <- function(fn, this_dir, out_dir, eval_metric, methods_use){
  setwd(this_dir)
  ...
  #temp<-ari_calcul_sampled_6(myData=thisData, cpcs=colPCA, isOptimal=TRUE,
  #                           method_use = methods_use,  
  #                           base_name=paste0(dataset_no, eval_metric, '_OP_'))
  setwd(paste0(out_dir, '/', eval_metric, "_CT"))
  temp<-ari_calcul_sampled_6(myData=thisData, cpcs=colPCA, isOptimal=FALSE, 
                             method_use = methods_use,  
